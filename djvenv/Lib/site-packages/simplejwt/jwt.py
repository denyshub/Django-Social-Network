...
from simplejwt.exception import (
...
def _hash(secret: bytes, data: bytes, alg: str) -> bytes:
    ...
    return hmac.new(secret, msg=data, digestmod=algorithm).digest()
...
class Jwt:
    ...
    def header(self) -> dict:
        ...
        header.update({"type": "JWT", "alg": self.alg})
        return header
    ...
    def _pop_claims_from_payload(self):
        ...
        """
        claims_in_payload = [
            k for k in self.payload.keys() if k in registered_claims.values()
        ]
    ...
    @staticmethod
    def decode(
        secret: Union[str, bytes], token: Union[str, bytes], alg: str = default_alg
    ) -> "Jwt":
        """
    ...
    def compare(self, jwt: "Jwt", compare_dates: bool = False) -> bool:
        ...
        if not compare_dates:
            strip = ["exp", "nbf", "iat"]
            expected_claims = {
                k: {v if k not in strip else None} for k, v in expected_claims.items()
            }
            actual_claims = {
                k: {v if k not in strip else None} for k, v in actual_claims.items()
            }
...
def compare_signature(expected: Union[str, bytes], actual: Union[str, bytes]) -> bool:
    """
...
def compare_token(expected: Union[str, bytes], actual: Union[str, bytes]) -> bool:
    """
