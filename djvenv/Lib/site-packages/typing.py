    ...
    * if Generic[...] is present, then it should list all type variables
    ...
    def __new__(
        ...
        super(GenericMeta, self).__setattr__(
            "_gorg", self if not origin else origin._gorg
        )
        ...
        for base in protocol_bases:
            for attr in base.__dict__.keys():
                ...
                for c in self.__mro__:
                    if (
                        c is not base
                        and attr in c.__dict__
                        and not getattr(c, "_is_protocol", False)
                    ):
                        break
                else:
                    if (
...
if hasattr(collections_abc, "Coroutine"):
    ...
    class Coroutine(
        Awaitable[V_co], Generic[T_co, T_contra, V_co], extra=collections_abc.Coroutine
    ):
        __slots__ = ()
...
class NamedTuple(metaclass=NamedTupleMeta):
    ...
    def __new__(*args, **kwargs):
        ...
        if args:
            ...
            warnings.warn(
                "Passing 'typename' as keyword argument is deprecated",
                DeprecationWarning,
                stacklevel=2,
            )
        else:
            raise TypeError(
                "NamedTuple.__new__() missing 1 required positional "
                "argument: 'typename'"
            )
        if args:
            try:
                ...
                raise TypeError(
                    "NamedTuple.__new__() takes from 2 to 3 "
            ...
            warnings.warn(
                "Passing 'fields' as keyword argument is deprecated",
                DeprecationWarning,
                stacklevel=2,
            )
        else:
